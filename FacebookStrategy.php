...
class FacebookStrategy extends OpauthStrategy{
		...
		if (array_key_exists('code', $_GET) && !empty($_GET['code'])){
			...
			if (!empty($results) && !empty($results['access_token'])){
				...
				$this->auth = array(
					...
					'info' => array(
						'name' => $me->name,
						'image' => 'https://graph.facebook.com/v2.0/'.$me->id.'/picture?type=square'
					),
					'credentials' => array(
	...
	private function me($access_token){
		$me = $this->serverGet('https://graph.facebook.com/v2.0/me', array('access_token' => $access_token), null, $headers);
		if (!empty($me)){
